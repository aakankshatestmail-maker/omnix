...
const OUT_DIR = path.resolve(import.meta.dirname, '..', 'public', 'hero')
const OUT = path.resolve(OUT_DIR, 'hero.mp4')

fs.mkdirSync(OUT_DIR, { recursive: true })

const PROMPT =
  'The chibi 3D robot character stands idle with very subtle breathing motion, blinks its smiling visor eyes once, then gives a small friendly wave with its right hand while its head tilts slightly. Soft, gentle, loopable idle animation. Keep the character, colors, proportions, helmet, visor eyes, and orange necktie identical to the source image. IMPORTANT: the background must be PURE SOLID BLACK (#000000) — completely uniform, no gradient, no ambient lighting on the background, no floor, no shadow cast on any surface behind or beneath the character. The character itself must remain fully lit and retain its original white/lavender body, cyan visor eyes, and orange tie — do NOT darken the character. Strong rim lighting on the character so its edges read clearly against the black background. Clean Pixar-style 3D render, no camera movement, no zoom, no text, no words.'
